...
class Manager[F[_]: MonadThrowable: Sequence: Timer](journal: Journal[F], logger: String => Unit) {
  ...
  def apply[A, B](serviceName: String)(process: ProcessBuilder[F, A, B]): A => F[B] = {
    val context = ServiceContext(serviceName, journal, logger, MonadError[F, Throwable], Timer[F])
...
object Manager {
  ...
  def apply[F[_]: MonadThrowable: Sequence: Timer](journal: Journal[F]): Manager[F] =
    new Manager[F](journal, _ => ())

  def apply[F[_]: MonadThrowable: Sequence: Timer](journal: Journal[F], logger: String => Unit): Manager[F] =
